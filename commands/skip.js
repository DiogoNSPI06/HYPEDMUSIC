...
module.exports = {
  name: "skip",
  description: "Passe para à próxima musica!",
...
const vote = message.client.vote.get(message.guild.id)
    ...
    return message.channel.send(embed);
       ...
       if(vote.voters.includes(message.author.id)) {
         return message.channel.send("Vc já votou")
       }
       
       if(vcvote === 2) {
          serverQueue.connection.dispatcher.end();
    embed.setDescription("✔ | Pulando a música!")
    embed.setThumbnail(client.user.displayAvatarURL())
    return message.channel.send(embed);
       }
...
vote.vote++
       ...
       return message.channel.send(`Vc precisa votar para skipar a música, ja temos ${Math.floor(vcvote - vote.vote)} votos`)
